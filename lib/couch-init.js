...
CouchInit.prototype.waitForCouch = function(couchUrl, timeout, attempts) {
    var requestPromises = [];
    var requestCouchInfoThisManyTimes = function requestCouchInfoThisManyTimes(times) {
        if (times > 0) {
            var options = {
                url: couchUrl,
                method: 'GET'
            };
            var couchInfoRequest = requestPromise(options);
            requestPromises.push(couchInfoRequest);
            requestCouchInfoThisManyTimes(times - 1); 
        }
        else return true;
    };
    requestCouchInfoThisManyTimes(attempts);
    return Q.all(requestPromises);
};
